// make-music.mjs — synthesize an exciting, modern/futuristic electronic BGM bed
// as a 16-bit stereo WAV. Pure Node, no ffmpeg / no ML models. Deterministic.
//
//   node scripts/make-music.mjs            -> assets/bgm/track.wav (60s)
//
// Arrangement (60s, 124 BPM, Am–F–C–G):
//   0–2s    atmospheric intro under the title fade-in (pad + soft arp)
//   2s      four-on-the-floor kick + sub bass + bright trance arp enter
//   ~6s     riser + impact  -> USE CASE 01 reveal
//   ~30s    riser + impact  -> USE CASE 02 reveal
//   ~54s    riser into outro
//   ~58s    final impact (crash swell + low boom)
// Energy is high and driving throughout; section impacts punctuate the two
// use-case reveals and the finale so the music lands with the visuals.
...
// section impact / reveal times (align with composition scene dividers)
const REVEALS = [6.0, 30.0];
...
// rising noise sweep (uplift before a section reveal)
function riser(start, dur) {
  add(start, dur, (t) => {
    const p = t / dur;
    const env = p * p; // accelerates toward the hit
    const cutoff = 0.2 + 0.8 * p;
    const n = rand() * 2 - 1;
    // pitched whoosh: rising sine bed + filtered noise
    const sweep = Math.sin(2 * Math.PI * (200 + 1400 * p) * t) * 0.18;
    const v = (n * cutoff * 0.18 + sweep) * env;
    return [v, v];
  });
}

function impact(start, big) {
  // crash-ish noise swell + low boom
  const dur = big ? 2.4 : 1.6;
  const ng = big ? 0.18 : 0.13;
  const bg = big ? 0.55 : 0.4;
  add(start, dur, (t) => {
    const env = Math.exp(-t * (big ? 1.5 : 2.0));
    const n = rand() * 2 - 1;
    const boom = Math.sin(2 * Math.PI * (40 + 30 * Math.exp(-t * 6)) * t) * Math.exp(-t * 3);
    const v = n * env * ng + boom * bg;
    return [v, v];
  });
}
...
for (let b = 0; b < nBars; b++) {
  ...
  for (let bt = 0; bt < 4; bt++) {
    ...
    // extra 16th hats for drive throughout the energetic body
    if (tt + beat * 0.75 < DUR) hat(tt + beat * 0.75, 0.07);
  }
}

// section risers + impacts on the two use-case reveals
for (const rv of REVEALS) {
  riser(Math.max(0, rv - 1.6), 1.6);
  impact(rv, false);
}

// outro riser + final impact
riser(Math.max(0, DUR - 6.0), 1.8);
impact(Math.max(0, DUR - 2.3), true);
...
for (let i = 0; i < N; i++) {
  ...
  for (const ch of [L, R]) {
    let v = tanh(ch[i] * norm * 1.05) * fade;
